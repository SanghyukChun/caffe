...
void BNLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->phase_ == TRAIN) {
    ...
    caffe_gpu_axpby(batch_mean_.count(), Dtype(decay_), batch_mean_.gpu_data(), 
        Dtype(1-decay_), this->blobs_[2]->mutable_gpu_data());
    caffe_gpu_axpby(batch_variance_.count(), Dtype(decay_), batch_variance_.gpu_data(), 
        Dtype(1-decay_), this->blobs_[3]->mutable_gpu_data());
  } else {
    caffe_copy(batch_mean_.count(), this->blobs_[2]->gpu_data(), batch_mean_.mutable_gpu_data());
    caffe_copy(batch_variance_.count(), this->blobs_[3]->gpu_data(), batch_variance_.mutable_gpu_data());
  }
