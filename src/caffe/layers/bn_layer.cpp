...
void BNLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  H_ = bottom[0]->height();
  W_ = bottom[0]->width();
  var_eps_ = this->layer_param_.bn_param().eps();
...
void BNLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->phase_ == TRAIN) {
    ...
    caffe_cpu_axpby(batch_mean_.count(), Dtype(decay_), batch_mean_.cpu_data(), Dtype(1-decay_), this->blobs_[2]->mutable_cpu_data());
    caffe_cpu_axpby(batch_variance_.count(), Dtype(decay_), batch_variance_.cpu_data(), Dtype(1-decay_), this->blobs_[3]->mutable_cpu_data());
  } else {
    caffe_copy(batch_mean_.count(), this->blobs_[2]->cpu_data(), batch_mean_.mutable_cpu_data());
    caffe_copy(batch_variance_.count(), this->blobs_[3]->cpu_data(), batch_variance_.mutable_cpu_data());
  }
